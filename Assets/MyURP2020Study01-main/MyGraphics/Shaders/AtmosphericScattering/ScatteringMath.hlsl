#ifndef __SCATTERING_MATH__
#define __SCATTERING_MATH__
...
float MiePhaseHG(float cosAngle, float g)
{
    float oneMinusG = 1 - g;
    float g2 = g * g;
    float a = (1.0 / (4.0 * PI)) * oneMinusG * oneMinusG;
    float b = pow(abs(1.0 + g2 - 2.0 * g * cosAngle), 3.0 / 2.0);
    return a / b;
}
...
{
    float g2 = g * g;
    float a = ((1.0 / (4.0 * PI)) * (3.0 * (1.0 - g2)) / (2.0 * (2.0 + g2)));
    float b = ((1 + cosAngle * cosAngle) / (pow(abs(1 + g2 - 2 * g * cosAngle), 3.0 / 2.0)));
    return a * b;
}
