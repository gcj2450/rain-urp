...
using System.Linq;
using MyGraphics.Scripts.CartoonWater;
using UnityEditor;
using UnityEngine;

namespace MyGraphics.Editor.CartoonWater
{
	...
	public class MyRenderObjectsPassFeatureEditor : PropertyDrawer
	{
		public class Styles
		{
			...
			public static GUIContent renderHeader = new GUIContent("Overrides",
				"Different parts of the rendering that you can choose to override.");

			//Filters
			public static GUIContent renderQueueFilter =
				new GUIContent("Queue", "Only render objects in the selected render queue range.");
			...
			public static GUIContent renderingLayerMask = new GUIContent("Rendering Layer Mask",
				"Only render objects in a rendering layer that match the given rendering layer mask.");

			public static GUIContent shaderPassFilter = new GUIContent("LightMode Tags",
				"Controls which shader passes to render by filtering by LightMode tag.");

			//Render Options
			public static GUIContent overrideMaterial = new GUIContent("Material",
				"Choose an override material, every renderer will be rendered with this material.");

			public static GUIContent overrideMaterialPass =
				new GUIContent("Pass Index", "The pass index for the override material to use.");
			...
			//Camera Settings
			public static GUIContent overrideCamera = new GUIContent("Camera",
				"Override camera matrices. Toggling this setting will make camera use perspective projection.");

			public static GUIContent cameraFOV = new GUIContent("Field Of View",
				"The camera's view angle measured in degrees along vertical axis.");

			public static GUIContent positionOffset = new GUIContent("Position Offset",
				"Position offset to apply to the original camera's position.");

			public static GUIContent restoreCamera = new GUIContent("Restore",
		...
		private void DoCameraOverride(ref Rect rect)
		{
			...
			if (m_OverrideCamera.boolValue)
			{
				...
				EditorGUI.BeginChangeCheck();
				var newOffset = EditorGUI.Vector3Field(rect, Styles.positionOffset,
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
		{
			...
			if (m_RenderFoldout.value)
			{
				height += Styles.defaultLineSpace *
				          (m_OverrideMaterial.objectReferenceValue != null ? m_MaterialLines : 1);
