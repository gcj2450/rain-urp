...
{
#if UNITY_EDITOR
	//camera synced with scene view
	[HideInInspector, SerializeField] private Camera syncedGameCamera;

	//transform backups (private, hidden)
	...
	//camera backups (private, hidden)
	[HideInInspector, SerializeField] private float defaultDepth;

	[HideInInspector, SerializeField] private bool orthographic;

	[HideInInspector, SerializeField] private float defaultOrthographicSize;

	[HideInInspector, SerializeField] private float defaultFieldOfView;

	[SerializeField] //settings
	private bool disableOnPlay;

	[SerializeField] public bool lockOnPlay;
	[SerializeField] public bool revertOnDestroy = true;
	...
	void SetUpCamera()
	{
		...
		syncedGameCamera.depth =
			camsDepth.Last() + 1f; //get highest depth and add +1 to make sure our camera will draw over antoher ones
	}
		...
		if (!Application.isPlaying || Application.isPlaying && !lockOnPlay) //lock in play mode if checked
		{
			if (SceneView.lastActiveSceneView != null && SceneView.lastActiveSceneView.camera == Camera.current) // Set alignment once, only after scene view has been rerendered
			{
				...
				else
				{
					...
					Debug.LogError("Scene View Cam: (" + this.gameObject.name +
					               ") Camera was removed. Removing script");
