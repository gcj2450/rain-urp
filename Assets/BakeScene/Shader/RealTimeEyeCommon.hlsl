#ifndef REALTIMEEYE_COMMON_INCLUDE
#define REALTIMEEYE_COMMON_INCLUDE

#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"

//获取屏幕空间ao
void GetSSAO(float2 ScreenUV, out float SSAO)
{
	SSAO = 1.0f;
...
float Vis_SmithJointApprox(float a2, float NoV, float NoL)
{
	...
	return 0.5 * rcp(Vis_SmithV + Vis_SmithL); //rcp(x) 1.0 / (x)
}
...
{
	return saturate(pow(max(NoV + AO, 0.001), RoughnessSq) - 1 + AO);
}
