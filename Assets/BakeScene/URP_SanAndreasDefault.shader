Shader "Custom/URP_SanAndreasDefault"
{
	Properties
	{
		...
		SubShader
		{
			...
			#pragma vertex vert alphatest:_AlphaCutoff
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				float3 Albedo = MainColor50.rgb*i.color.rgb;//=======================
				...
				half noise = tex2D(_NoiseTex, i.screenPos.xy / (i.screenPos.w == 0 ? 1 : i.screenPos.w)).a * .99;
				half fade = half(_Fade < 0 ? noise > 1 + _Fade : noise < _Fade);


				float Alpha =  tex2D(_MaskTex, uv_MainTeture).a* fade * i.color.a * _MainColor.a;//================
