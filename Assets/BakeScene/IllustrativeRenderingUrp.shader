...
Shader "Custom/IllustrativeRenderingUrp" 
{
	Properties 
	{
		...
		_SpecularPower("Specular Power", float) = 100.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				half4 c;
				c.rgb = ((albedo.rgb* _Color.rgb) + (mainLight.shadowAttenuation *mainLight.distanceAttenuation  * mainLight.color.rgb * NdotL)+ ( _SpecularColor.rgb)) ;
				c.a = albedo.a;
