...
float4 CalculateSSSDiffuse(CusomLightingData lightingData, float curveValue)
{
    return SAMPLE_TEXTURE2D(_SSSTex, sampler_SSSTex, float2(lightingData.NoL * 0.6 + 0.4, curveValue));
}

float CalculateBasicDiffuse(CusomLightingData lightingData, VertexOutput vertexOutput)
{
    //Disney Diffuse
...
float4 CalculateKelemenSpecular(CusomLightingData lightingData, VertexOutput vertexOutput)
{
...
float4 Frag(VertexOutput vertexOutput) : SV_Target
{
    ...
    CusomLightingData lightingData;
